 ...
 *   OR ANTHROPIC_OAUTH_TOKEN: OAuth token for Claude Pro/Max subscribers (generate via `claude setup-token`)
 *
 * Optional secrets:
...
// Middleware: Validate required environment variables (skip in dev mode, debug routes, and admin API routes)
app.use('*', async (c, next) => {
  const url = new URL(c.req.url);
  ...
  // Skip validation for admin API routes (they use gateway token auth)
...
app.use('*', async (c, next) => {
  ...
  // Skip global CF Access for /api/admin/* routes - they have their own middleware with gateway token auth
