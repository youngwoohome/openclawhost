...
export default function AdminPage() {
  ...
  const [pairingChannel, setPairingChannel] = useState<string>('telegram')
  ...
  const handleApprovePairing = async () => {
    if (!pairingCode.trim()) {
      setPairingResult({ success: false, message: 'Please enter a pairing code' })
      return
    }
    ...
    try {
      const result = await approveChannelPairing(pairingChannel, pairingCode.trim().toUpperCase())
      if (result.success) {
        setPairingResult({ success: true, message: result.message || 'Pairing approved!' })
        setPairingCode('') // Clear the input on success
      } else {
        setPairingResult({ success: false, message: result.error || result.message || 'Approval failed' })
      }
    } catch (err) {
      setPairingResult({
        success: false,
  ...
  return (
    <div className="devices-page">
      ...
      <section className="devices-section">
        ...
        <p className="hint">
          Approve pairing requests from Telegram, Discord, or Slack. Enter the pairing code shown in the channel.
        </p>

        <div className="pairing-form">
          <div className="pairing-inputs">
            <select
            ...
            <input
              ...
              placeholder="Enter pairing code (e.g., YHBLUHB7)"
          ...
          {pairingResult && (
            <div className={`pairing-result ${pairingResult.success ? 'success' : 'error'}`}>
