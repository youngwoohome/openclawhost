...
adminApi.use('*', createAccessMiddleware({ type: 'json', allowGatewayToken: true }));
...
// POST /api/admin/pairing/approve - Approve a channel pairing request (Telegram, Discord, Slack)
adminApi.post('/pairing/approve', async (c) => {
  ...
  try {
    ...
    if (!channel || typeof channel !== 'string') {
      return c.json({ error: 'channel is required (telegram, discord, or slack)' }, 400);
    }

    if (!code || typeof code !== 'string') {
      return c.json({ error: 'code is required (pairing code from the channel)' }, 400);
    }

    // Validate channel type
    const validChannels = ['telegram', 'discord', 'slack'];
    if (!validChannels.includes(channel.toLowerCase())) {
      return c.json({ error: `Invalid channel: ${channel}. Must be one of: ${validChannels.join(', ')}` }, 400);
    }

    // Validate code format (alphanumeric, typically 8 chars)
    if (!/^[A-Za-z0-9]+$/.test(code)) {
      return c.json({ error: 'Invalid code format. Code must be alphanumeric.' }, 400);
    }

    // Ensure moltbot is running first
    await ensureMoltbotGateway(sandbox, c.env);

    // Run clawdbot CLI to approve the channel pairing
    const proc = await sandbox.startProcess(`clawdbot pairing approve ${channel.toLowerCase()} ${code} --url ws://localhost:18789`);
    await waitForProcess(proc, CLI_TIMEOUT_MS);
    ...
    // Check for success - look for approval confirmation or exit code 0
    ...
    return c.json({
      ...
      code,
      message: success ? `${channel} pairing approved successfully` : 'Approval may have failed',
      stdout,
      stderr,
    });
  } catch (error) {
