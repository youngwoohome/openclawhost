...
export async function ensureMoltbotGateway(sandbox: Sandbox, env: MoltbotEnv): Promise<Process> {
  ...
  try {
    ...
    let processLogs: { stdout?: string; stderr?: string } = {};
    try {
      processLogs = await process.getLogs();
      console.error('[Gateway] startup failed. Stderr:', processLogs.stderr);
      console.error('[Gateway] startup failed. Stdout:', processLogs.stdout);
    } catch (logErr) {
      console.error('[Gateway] Failed to get logs:', logErr);
    }
    // Include both stdout and stderr in error message for debugging
    const stderr = processLogs.stderr || '(empty)';
    const stdout = processLogs.stdout || '(empty)';
    throw new Error(`Moltbot gateway failed to start.\n\nSTDERR:\n${stderr}\n\nSTDOUT:\n${stdout}`);
  }
